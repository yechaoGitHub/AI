...
HttpClientPrivateCache HttpClientPrivate::cache() {
    ...
    cache.timeoutHandler = timeoutHandler;
    cache.debug = debug;
...
void HttpClientPrivate::executeQuery(HttpClientPrivate* d, HttpClientRequestMethod method) {
    ...
    QEventLoop loop;
    QTimer timer;
    timer.setInterval(d->timeout*1000);
    ...
    // [4] 请求结束时获取响应数据，在 handleFinish 中执行回调函数
    // 请求结束时一次性读取所有响应数据
    QObject::connect(reply, &QNetworkReply::finished, [=,&loop,&timeOut] {
        if (timeOut) {
            return;
        }
        loop.quit();
    ...
    if (timer.isActive()) {
        ...
        if (cache.debug) {
            qDebug().noquote() << QString("[超时] 失败");
        }

        // [2] 执行请求失败的回调函数
        if (nullptr != cache.timeoutHandler) {
            cache.timeoutHandler();
        }
...
// 注册请求超时的回调函数
HttpClient& HttpClient::timeout(std::function<void()> timeoutHandler) {
    d->timeoutHandler = timeoutHandler;
